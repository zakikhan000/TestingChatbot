...
database = 'Anonymous'  # Replace with your actual database name
username = 'zaki'
password = '12365'
...
def add_user_authentication():
    ...
    return jsonify({'message': 'User authentication record added successfully!'})
...
@app.route('/auth/<int:uaid>', methods=['DELETE'])
def delete_user_authentication(uaid):
    conn = get_db_connection()
    cursor = conn.cursor()

    cursor.execute('DELETE FROM UserAuthentication WHERE UAID = ?', (uaid,))
    conn.commit()
    conn.close()
...
def get_combined_data():
    ...
    LEFT JOIN 
        Users u ON ua.UAID = u.UAID
    LEFT JOIN 
        UserProfileImage up ON u.UTID = up.UTID
    '''
